...
const previousSnapshot = await readGeneratedExport(new URL("../src/open-english-dictionary-meta.js", import.meta.url), "englishDictionarySnapshot");
...
const [dictionaryChanged, metadataChanged] = await Promise.all([
  writeTextIfChanged(outputPath, moduleSource),
  writeTextIfChanged(metaOutputPath, metaSource)
]);
