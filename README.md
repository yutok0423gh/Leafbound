...
Leafbound 是依照提供的 Product Requirements Document 建立的個人語言書房，中文名為「拾頁」。這是一個無需帳戶、無需後端的 local-first 瀏覽器 App，資料只保存在目前瀏覽器的 `localStorage`。
...
- 粵語：36 個站內條目，包括 5 段 HKCanCor 真人粵語原聲、28 篇橫跨 Level 1–7 的冚唪唥完整粵文故事及 3 篇本地練習；支援內容書架與等級篩選、逐字稿點詞查音、語料原有粵拼、分級故事自動粵拼與獨立開關、原聲／朗讀狀態、播放進度、速度、前後 10 秒、AB Repeat、三種 Transcript 模式與詞語收藏。另有 62,274 個粵典候選讀音，以及 26,983 個 Rime Cantonese 單字候選用於罕見字補充
- English：3 篇本地精讀稿，以及由 VOA Learning English、NASA、Standard Ebooks 官方 feed 生成的站內正文書架；VOA／NASA 收錄清洗後全文，Standard Ebooks 收錄首章。支援按來源／語言／文化／科學／文學／生活分類、文章閱讀、字級與行距、夜間閱讀、閱讀進度、逐詞點擊、已有詞義提示、英文發音、word / phrase / sentence 語境收藏與文章筆記
- 我的：跨模組 Library、篩選、收藏與筆記、資料持久化
...
- 粵語內容匯入器讀取冚唪唥 208 篇公開分級目錄，現時每級選入 4 篇同時具有完整文本、逐篇 CC BY 署名與真人原聲入口的故事，共 28 篇；正文粵拼由本地詞表優先整詞匹配、再以單字表補齊，預設顯示首個候選並清楚標示為自動標註。另收錄 HKCanCor 官方提供的 5 段 CC BY 4.0 真人錄音樣本、說話者逐字稿及語料原有粵拼。HKCanCor 句段時間是按文字長度估算，不冒充人工對軸。
...
測試涵蓋詩／詞／古文數量與來源完整性、宋詞繁體轉換、古文分段與全庫漢字粵拼覆蓋、粵典最長詞匹配、Rime Cantonese 罕見字回退、HKCanCor 本機音訊與粵拼完整性、冚唪唥七級數量與逐篇授權、English 公開正文的清洗與授權邊界、local persistence、收藏切換、去重收藏與進度計算；完整瀏覽器驗收另涵蓋真人粵語播放、內容書架／等級篩選、古文粵拼開關與沉浸閱讀、公開英文正文站內閱讀、粵語／古典原文點詞、來源標示、詞彙收藏標記、組合篩選、詩脈／文脈，並逐頁檢查 320px 手機、手機橫屏、820px Pad 直向、1024px Pad 橫向及 1440px 電腦版面。
