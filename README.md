...
內容更新排程為每週一 01:00 UTC（北京／上海／香港時間 09:00），同時為粵語及 English 各收錄最多 20 篇從未收錄過的文章。GitHub Actions 排程可能因平台排隊而延後，09:00 是預定啟動時間。亦可在 **Refresh Leafbound content** 頁面手動選擇 `all`、`english` 或 `cantonese`；同週重跑只補足該語言的剩餘名額，不會再次新增 20 篇。發布週期由週一 09:00 起計。

永久收錄記錄保存在 `data/content-history.json`，首次由完整 Git 歷史中的英文、粵語及訪談快照建立，包含曾經收錄後被舊版更新移出的文章。去重同時核對 ID、標準化來源網址、原文文件 ID、同來源標題及正文指紋。之後新增內容保留在書架中，舊收藏及文章連結繼續有效。每種語言只接受具備正文及來源資料的新內容，粵文另檢查本地粵拼是否可用；冚唪唥會檢查整個分級目錄中的未收錄故事，不再只取每級固定前 24 篇。固定訪談錄音不計入每週新增文章名額。

冚唪唥新故事不足時，會透過粵語維基百科官方 API 尋找本站從未收錄的百科文章。僅接受確認為粵語、CC BY-SA 4.0、含足夠完整段落及可用粵拼的純文字內容，排除短小條目、列表及消歧義頁，保留原文、版本連結及作者署名。百科文章使用裝置的粵語合成朗讀，不會標示為真人錄音。

來源不足時不拿舊文湊數：網站的「本週上新」／“This week's new texts” 顯示本期文章、實際新增數、缺額及檢查日期，GitHub 執行摘要也會列明缺額。上游文章、正文和授權可用性會影響實際數量；任何來源無法保證每週必有 20 篇合格新文。正常缺額不會被當成執行失敗；所有來源無法讀取或資料驗證失敗時仍會保留舊版。

匯入後會重建 English 本地點詞詞庫並執行完整單元測試；驗證失敗時，產生檔、每週發布清單、永久去重記錄與本機音訊一起回復原狀，避免尚未發布的文章被誤記為已收錄。成功提交後會接續觸發 GitHub Pages 部署。英文及粵語原始匯入腳本必須透過 `content:update` 統一命令執行，以確保上述回復機制生效。
...
英文詞典的中文釋義比例及全庫查詞覆蓋率會隨新增文章的人名、地名和專有詞彙改變；分別低於或等於 88% 及 90% 的參考值時，匯入日誌及 GitHub Actions 會提示 warning。固定 Leafbound 精讀稿的查詞覆蓋率至少 98%、既有詞典規模、核心詞義，以及生成詞條與統計資料的一致性仍由測試驗證；驗證失敗仍會回復更新前的內容。
